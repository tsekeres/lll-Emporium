...
namespace LLL_Emporium.Controllers
{
    ...
    public class OrderLineController : ControllerBase
    {
        ...
        [HttpGet("/api/orders/{orderId}/orderLinesWithProduct")]
        public IActionResult GetOrderLinesWithProduct(Guid orderId)
        {
            var result = _orderLineRepository.GetOrderLinesWithProduct(orderId);
            if (result != null)
            {
                return Ok(result);
            }
        ...
        [HttpGet("/api/orders/orderLinesWithProduct/{productId}")]
        public IActionResult GetOrderLinesWithProductById(Guid productId)
        {
            var result = _orderLineRepository.GetOrderLinesWithProduct(productId);
            if (result != null)
            {
                return Ok(result);
            }
            else return NotFound($"Order lines for order {productId} not found.");
        }
